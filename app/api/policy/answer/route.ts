...
- 특히 사용자가 "다리가 문질러졌다", "문질러졌고"라고 질문한 경우, 이는 100% "다리가 부러졌다(골절)" 또는 "부상을 당했다"의 음성 인식(STT) 오류입니다. 답변의 [분석 배경 및 이해] 및 전체 텍스트 내에서 절대 "문질러졌다"라는 단어를 그대로 기재하지 말고, 반드시 "다리가 부러졌다(골절)" 또는 "다쳤다"로 문맥을 완전하게 정정하여 신뢰성 있고 매끄러운 분석서를 작성하십시오.
