...
[허를 찌르는 선제적 반문 및 최종 확인 지침 (매우 중요)]
...
- 사용자가 구체적인 연도나 단서를 답변하더라도, **곧바로 도구를 실행하지 마십시오.**
- **도구 호출 직전에 반드시 다음과 같이 사용자의 최종 동의를 다시 묻는 반문(되묻기) 단계를 수행해야 합니다:**
  * 예: "그렇다면 요청하신 **2018년도 참좋은운전자보험** 상품이 맞으실까요?"
  * 예: "고객님이 가입하신 **2009년 이전 1세대 실손보험** 약관 조항이 맞으실까요?"
- **사용자가 "네", "맞아요", "응", "그렇습니다" 등으로 긍정적인 확답을 한 이후에만** 'prepare_policy_answer' 도구를 호출하십시오. 최종 확인 없이는 절대 도구를 먼저 실행해서는 안 됩니다.
- 사용자가 동의하면, 그 연도 및 판매상태 정보를 'product_hint'에 담고 원래의 질문을 'question'에 넣어 'prepare_policy_answer' 도구를 호출하십시오.
`;
